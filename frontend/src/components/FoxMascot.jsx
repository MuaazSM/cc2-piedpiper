...
export default function FoxMascot({ size = 'md', variant = 'idle', className = '' }) {
  ...
  return (
    <>
      <style>{`
        .fox-float  { animation: foxFloat 3.5s ease-in-out infinite; }
        .fox-tilt   { animation: foxTilt  2s ease-in-out infinite; }
        .fox-bounce { animation: foxBounce 0.6s ease-in-out infinite alternate; }
        @keyframes foxFloat {
        ...
        .fox-glow { animation: foxGlow 3.5s ease-in-out infinite; }
        @keyframes foxGlow {
          0%,100% { filter: drop-shadow(0 0 3px rgba(var(--page-glow-rgb), 0.4)); }
          50%     { filter: drop-shadow(0 0 10px rgba(var(--page-glow-rgb), 0.9))
                            drop-shadow(0 0 20px rgba(var(--page-glow-rgb), 0.4)); }
        }
        .fox-wrap {
          position: relative;
          display: inline-flex;
        ...
        .fox-spin {
          animation: foxSpin 0.6s cubic-bezier(0.4, 0, 0.2, 1) forwards !important;
        }
        @keyframes foxSpin {
      ...
      <div
        className={`fox-wrap ${spinning ? '' : anim}`}
        onClick={handleClick}
      >
        <div
          className={`inline-block fox-glow ${spinning ? 'fox-spin' : ''} ${className}`}
          style={{ width: w, height: h }}
          onAnimationEnd={(e) => {
            if (e.animationName === 'foxSpin') setSpinning(false)
          }}
        >
          <svg
          ...
          >
            ...
            <path d="M559.499,248.489c-19.34-18.502-29.304-36.785-28.814-52.867c0.3-9.811,4.487-15.703,4.529-15.752l2.861-3.509l0.36-4.511
              ...
              C229.293,314.950,233.107,320.966,231.787,324.327z"/>
          </svg>
        </div>
      </div>
    </>
